...
var component_type_definition = ComponentTypeDefinition.new(Enums.ComponentType.INPUT, "", Enums.GatePredicateType.UNEDEFINED, 1, 1)

# empty array for array, as there are no input components, just an input value
func _init(initial_value: bool, identifier: String = "").(component_type_definition, process_delegate, [], identifier):
